...
```
python /path_to_repo/gasp/scripts/run.py input_file.yaml
```

where 'path_to_repo' is the path to the GASP-python repository on your computer and 'input_file.yaml' is the main input file read by GASP that contains the parameters for the structure search. It must be in [YAML](https://en.wikipedia.org/wiki/YAML) format and end with '.yaml'. 
...
Specifies the standard deviation of the Gaussian distribution from which the number of stoichiometries' worth of atoms to add to (or remove from) a parent structure is drawn. Optional, and defaults to 1.0. 
...
Much important information about the progress of the algorithm is written to screen. This text is not saved elsewhere, but it may be captured with, for example, the `tee` utility on the UNIX command line. For important runs, it is recommended to save this output to disk for possible future analysis. 
...
It is sometimes useful to visualize the numbers of atoms in the structures generated by the algorithm. A Python script to plot of the number of atoms in each structure versus the number of completed energy calculations using [pymatgen](http://pymatgen.org) and the [matplotlib](http://matplotlib.org) plotting library is located [here](../gasp/scripts/plot_system_size.py). It takes the 'run_data' file as an argument (see [Output to disk](#diskoutput)). 

For phase diagram searches, we often want to view the phase diagram predicted by the algorithm. A Python script to plot the phase diagram using [pymatgen](http://pymatgen.org) and the [matplotlib](http://matplotlib.org) plotting library is located [here](../gasp/scripts/plot_phase_diagram.py). It takes the 'run_data' file as an argument (see [Output to disk](#diskoutput)). 
