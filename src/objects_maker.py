...
def make_objects(parameters):
    ...
    if 'Constraints' in parameters:
        ...
        # if no Constraints block is given in the input file, then just use
    ...
    if 'Geometry' in parameters:
        ...
        # if no Geometry block is given in the input file, then just use
        # default values for everything
    ...
    if 'Development' in parameters:
        ...
        # if no Development block is given in the input file, then just use
        # default values for everything
    ...
    # if no method is specified for making the initial population, throw an
    # error if we're doing a phase diagram search
    ...
    # if the InitialPopulation block is given but left blank or default, throw
    # an error if we're doing a phase diagram search
        ...
        # if path_to_folder flag is not given
            ...
            # if the folder exists, check that it contains files
                ...
                files_organism_creator = organism_creators.FileOrganismCreator(
                    given_path)
                # check that the provided files cover all the composition space
                # endpoints
                if composition_space.objective_function == 'pd':
                    # all the structures created from the provided files
                    structures = files_organism_creator.get_structures()
                    # list to hold the composition space endpoints for which
    ...
    if 'NumCalcsAtOnce' in parameters:
        if parameters['NumCalcsAtOnce'] in (None, 'default'):
            num_calcs_at_once = 1
        else:
    ...
    if 'RunTitle' in parameters:
        if parameters['RunTitle'] is None:
            run_title = ''
        else:
            run_title = parameters['RunTitle']
    else:
        # if no RunTitle block is given in the input file, set it to an empty
        # string
        run_title = ''
    ...
    if 'EnergyCode' not in parameters:
        ...
        if parameters['EnergyCode']['gulp'] is None:
            ...
            if 'potential_file' not in parameters['EnergyCode']['gulp']:
                ...
                if not os.path.exists(potential_file_path):
                    ...
                    print('Quitting...')
                    quit()
            # if we made it this far, then both the header and potential file
            # exist, so make the gulp energy calculator
        ...
        if parameters['EnergyCode']['lammps'] is None:
            ...
            if 'input_script' not in parameters['EnergyCode']['lammps']:
                ...
                if not os.path.exists(input_script_path):
                    ...
                    print('Quitting...')
                    quit()
            # if we made it this far, then the input script exists, so make the
            # lammps energy calculator
        ...
        if parameters['EnergyCode']['vasp'] is None:
            ...
            if 'potcars' not in parameters['EnergyCode']['vasp']:
                ...
                # get the dictionary containing the paths to the POTCAR files
                # of each element
                ...
                for key in potcar_paths:
                    if potcar_paths[key] is None:
                        ...
                        print('Quitting...')
                        quit()
            # if we made it this far, then the all the files were provided
            # properly and exist, so make the vasp energy calculator
    ...
    if 'StoppingCriteria' in parameters:
        # if the entire block is empty or set to default, then set everything
        # to default
        if parameters['StoppingCriteria'] in (None, 'default'):
            stopping_criteria = general.StoppingCriteria(None,
                                                         composition_space)
        # need to check that if the found structure option is used and a file
        # path is given, that it's valid
        elif 'found_structure' in parameters['StoppingCriteria']:
            if parameters['StoppingCriteria']['found_structure'] in (
                    None, 'default'):
                ...
                # check that the file has the correct suffix or prefix
                elif given_path.endswith('.cif') or given_path.startswith(
                        'POSCAR'):
                    ...
                    # couldn't read the structure from the given file
                        ...
                        print('Error reading the structure to find from the '
                              'given file.')
                        print('Quitting...')
                        quit()
                else:
                    print('File containing structure to find must be in '
                          'POSCAR or cif format and begin with POSCAR or end '
                          'with .cif, respectively.')
                    print('Quitting...')
                    quit()
        else:
            stopping_criteria = general.StoppingCriteria(
    ...
    if 'Variations' not in parameters:
        ...
        structure_mut = variations.StructureMut(
            {'fraction': default_structure_mut_fraction})
        num_stoichs_mut = variations.NumStoichsMut(
            {'fraction': default_num_stoichs_mut_fraction})
        permutation = variations.Permutation(
            {'fraction': default_permutation_fraction}, composition_space)
        ...
        if 'Mating' not in parameters['Variations']:
            ...
            # Mating sub block was used is not empty. Check that the
            # non-optional 'fraction' flag specifies a valid value
        ...
        if 'StructureMut' not in parameters['Variations']:
            # StructureMut sub block was not used, so don't make a StructureMut
            # variation
            pass
            ...
            # StructureMut sub block was used is not empty. Check that the
            # non-optional 'fraction' flag specifies a valid value
            if parameters['Variations']['StructureMut']['fraction'] in (
                ...
                # make a StructureMut variation object with the supplied
                # parameters
                structure_mut = variations.StructureMut(
        ...
        if 'NumStoichsMut' not in parameters['Variations']:
            # NumStoichsMut sub block was not used, so don't make a
            ...
            # NumStoichsMut sub block was used is not empty. Check that the
            # non-optional 'fraction' flag specifies a valid value
            if parameters['Variations']['NumStoichsMut']['fraction'] in (
                ...
                # make a NumStoichsMut variation object with the supplied
                # parameters
                num_stoichs_mut = variations.NumStoichsMut(
        ...
        if 'Permutation' not in parameters['Variations']:
            # Permutation sub block was not used, so don't make a Permutation
            # variation
            pass
            ...
            # Permutation sub block was used is not empty. Check that the
            # non-optional 'fraction' flag specifies a valid value
            if parameters['Variations']['Permutation']['fraction'] in (
                ...
                # make a Permutation variation object with the supplied
                # parameters
                permutation = variations.Permutation(
