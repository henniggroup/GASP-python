...
This module contains the classes used to create organisms for the initial population.

1. RandomOrganismCreator: creates random organisms
2. FileOrganismCreator: creates organisms by reading their structures from files 
...
class FileOrganismCreator(object):
    ...
    def __init__(self, path_to_folder):
        ...
        self.files = [f for f in os.listdir(self.path_to_folder) if os.path.isfile(os.path.join(self.path_to_folder, f))]
    ...
    def create_organism(self, id_generator, composition_space, constraints, random):
        ...
        TODO: the last three arguments are never actually used in this method, but I included them so the method has the same arguments as RandomOrganismCreator.creatorOrganism()
              to allow the create_organism method to be called on both RandomOrganismCreator and FileOrganismCreator without having to know in advance which one it is.
