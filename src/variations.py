...
This module contains the classes used to create offspring organisms from parent organisms.

1. Mating: creates an offspring organism by combining two parent organisms
2. StructureMut: creates an offspring organism by mutating the structure of a parent organism
3. NumStoichsMut: creates an offspring organism by adding or removing atoms from a parent organism
4. Permutation: creates an offspring organism by swapping atoms in a parent organism
...
class Mating(object):
    ...
    def __init__(self, mating_params):
        ...
        self.default_merge_cutoff = 1.0 # the cutoff distance (as fraction of atomic radius) below which to merge sites with the same element in the offspring structure
        ...
        elif mating_params['merge_cutoff'] == None or mating_params['merge_cutoff'] == 'default':
            # use the default value if the flag was left blank or set to 'default'
            self.merge_cutoff = self.default_merge_cutoff
        else:
            # otherwise, parse the value from the parameters
    ...
    def do_variation(self, pool, random, geometry, id_generator):
        ...
        while len(species_from_parent_1) == 0 or len(species_from_parent_2) == 0:
            ...
            if random.random() < self.shift_prob:
                shifted_parent_1 = self.do_random_shift(parent_1, cut_vector_index, geometry, random)
        
            # possibly shift the atoms in the second parent along the cut vector
            if random.random() < self.shift_prob:
                shifted_parent_2 = self.do_random_shift(parent_2, cut_vector_index, geometry, random)
    ...
    def do_random_shift(self, organism, lattice_vector_index, geometry, random):
        '''
        ...
        Organism.translate_atoms_into_cell() to make sure all the atoms are located inside the cell.
        ...
        # otherwise, shift all the atoms along the specified lattice vector by a random amount
            ...
            shifted_org.translate_atoms_into_cell()
            return shifted_org
    ...
    def merge_sites(self, structure):
        ...
        # if any merges were done, call merge_sites recursively on the new structure
