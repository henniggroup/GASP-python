...
# the number of energy calculations completed. This gets incremented for both
# successful and unsuccessful energy calculations
...
# dictionary to temporarily hold the relaxed organisms. The key to each relaxed
# organism is the index of the Thread in the list threads that did the energy
# calculation
relaxed_organisms = {}
...
for creator in organism_creators:
    ...
    while not creator.is_finished:
        ...
        # start by doing num_calcs_at_once energy calculations, each on its own
        # thread
        if len(threads) < num_calcs_at_once:
            ...
            # this check is needed because the loop above could be exited with
            # a None organism for attempts-based creators
            if new_organism is not None:
                ...
                developed_org = developer.develop(
                    new_organism, composition_space, constraints, geometry,
                    pool)
                if developed_org is not None:  # successful development
                    # check for redundancy
                    redundant_org = redundancy_guard.check_redundancy(
                        developed_org, whole_pop)
                    if redundant_org is None:  # no redundancy
                        # we want to add copies to whole_pop so the organisms
                        # in whole_pop don't change upon relaxation, etc.
                        whole_pop.append(copy.deepcopy(developed_org))
                        # if num calcs is one of the stopping criteria, this
                        # updates it
                        stopping_criteria.update_calc_counter()
                        # for bulk search, this does nothing except rotate into
                        # principal directions
                        geometry.pad(developed_org)
                        # the index this thread will have once it's appended to
                        # the list threads
                        thread_index = len(threads)
                        thread = threading.Thread(
                            target=energy_calculator.do_energy_calculation,
                            args=[developed_org, relaxed_organisms,
                                  thread_index])
                        thread.start()
            ...
            for thread in threads:
                if not thread.is_alive():
                    ...
                    # remove the relaxed organism from the dictionary since we
                    # got it out
                    relaxed_organisms[thread_index] = None
                    # if the relaxed organism is not None, then do development
                    # and redundancy checking
                    if relaxed_org is not None:
                        # for bulk search, this does nothing
                        geometry.unpad(relaxed_org, constraints)
                        developed_org = developer.develop(
                            relaxed_org, composition_space, constraints,
                            geometry, pool)
                        if developed_org is not None:
                            redundant_org = redundancy_guard.check_redundancy(
                                developed_org, whole_pop)
                            if redundant_org is not None:
                                if redundant_org.is_active and \
                                        redundant_org.epa > developed_org.epa:
                                    initial_population.replace_organism(
                                        redundant_org, developed_org,
                                        composition_space)
                                    # print out info on the best organism and
                                    # how many calcs have been done so far
                                ...
                                # if value achieved or found structure stopping
                                # criteria are used, this checks if it's met
                                stopping_criteria.check_organism(developed_org)
                                initial_population.add_organism(
                                    developed_org, composition_space)
                                whole_pop.append(developed_org)
                                # print out info on the best organism and how
                                # many calcs have been done so far
                                initial_population.print_progress(
                                    composition_space)
                                print('Number of energy calculations so far: '
                                      '{} '.format(num_finished_calcs))
                                # if this organism was made by the current
                                # creator and the creator is success-based,
                                # then update the status of the creator
                                if creator.is_successes_based and \
                                        developed_org.made_by == creator.name:
                    ...
                    while not started_new_calc and not creator.is_finished:
                        ...
                        # this check is needed because the loop above could be
                        # exited with a None organism for attempts-based
                        # creators
                        if new_organism is not None:
                            # unpad the organism (does nothing for bulk search)
                            geometry.unpad(new_organism, constraints)
                            # develop the organism
                            developed_org = developer.develop(
                                new_organism, composition_space, constraints,
                                geometry, pool)
                            if developed_org is not None:
                                # check for redundancy
                                redundant_org = \
                                ...
                                if redundant_org is None:  # no redundancy
                                    ...
                                    thread = threading.Thread(
                                        ...
                                        args=(developed_org, relaxed_organisms,
                                              thread_index))
                                    thread.start()
                                    # replace the dead thread in threads with
                                    # the new one
                                    threads[thread_index] = thread
                                    # set the flag so we know we've started a
                                    # new energy calculation
                                    started_new_calc = True

# get the output of all the calculations that were still running when the last
# creator finished
...
while num_to_get > 0:
    for thread in threads:
        ...
        if not thread.is_alive() and thread_index not in handled_indices:
            ...
            # remove the relaxed organism from the dictionary since we got it
            # out
            relaxed_organisms[thread_index] = None
            # if the relaxed organism is not None, then do development and
            # redundancy checking
            if relaxed_org is not None:
                # for bulk search, this does nothing
                geometry.unpad(relaxed_org, constraints)
                developed_org = developer.develop(
                    relaxed_org, composition_space, constraints, geometry,
                    pool)
                if developed_org is not None:
                    redundant_org = redundancy_guard.check_redundancy(
                        developed_org, whole_pop)
                    if redundant_org is not None:
                        if redundant_org.is_active and redundant_org.epa > \
                                developed_org.epa:
                            initial_population.replace_organism(
                                redundant_org, developed_org,
                                composition_space)
                        ...
                        # criteria are used, this checks if it's met
                        stopping_criteria.check_organism(developed_org)
                        initial_population.add_organism(developed_org,
                                                        composition_space)
                        whole_pop.append(developed_org)
                        # print out info ont he best organism and how many
                        # calcs have been done so far
                        initial_population.print_progress(composition_space)
                        print('Number of energy calculations so far: '
                              '{} '.format(num_finished_calcs))


# add the initial population to the pool (this prints out a summary of the
# initial population)
...
for _ in range(num_calcs_at_once):
    ...
    stopping_criteria.update_calc_counter()
    # the index this thread will have once it's appended to the list threads
    thread_index = len(threads)
    thread = threading.Thread(
        target=energy_calculator.do_energy_calculation,
        ...
        if not thread.is_alive():
            ...
            if relaxed_org is not None:
                ...
                if developed_org is not None:
                    ...
                    redundant_org = redundancy_guard.check_redundancy(
                        developed_org, pool.to_list())
                    if redundant_org is not None:
                        if redundant_org.epa > developed_org.epa:
                            # replace the organism
                            pool.replace_organism(redundant_org, developed_org,
                                                  composition_space)
                            ...
                            # print out the progress of the search - either the
                            # best value (for epa) or the volume of the convex
                            # hull (for pd)
                            pool.print_progress(composition_space)
                            # print out how many energy calculations have been
                            # done so far
                            print('Number of energy calculations so far: '
                                  '{} '.format(num_finished_calcs))
                    # if not redundant with an organism in the pool, check for
                    # redundancy with all the organisms in whole_pop
                    else:
                        redundant_org = redundancy_guard.check_redundancy(
                            developed_org, whole_pop)
                    # if no redundancy in either the pool or whole_pop, then
                    # add the new organism to the pool
                    if redundant_org is None:
                        # if value achieved or found structure stopping
                        # criteria are used, this checks if it's met
                        stopping_criteria.check_organism(developed_org)
                        ...
                        pool.print_summary(composition_space)
                        pool.print_progress(composition_space)
                        # print out how many energy calculations have been done
                        # so far
                        print('Number of energy calculations so far: '
                              '{} '.format(num_finished_calcs))

            # make another offspring organism if the stopping criteria aren't
            # satisfied
...
# get the output of all the calculations that were still running when the
